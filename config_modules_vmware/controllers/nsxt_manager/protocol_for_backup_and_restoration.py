...
class ProtocolForBackupAndRestoration(BaseController):
    """Manage NSX-T backup on SFTP server .
    This is a controller implementation for nsxt manager.

    | Config Id - 0000
    | Config Title - manage configuration of SFTP backup server in NSX-T 

    """

    metadata = ControllerMetadata(
        name="protocol_for_backup_and_restoration",  # controller name
        path_in_schema="compliance_config.nsxt_manager.protocol_for_backup_and_restoration",  # path in the schema to this controller's definition.
        configuration_id="0000",  # configuration id as defined in compliance kit.
        title="Manage NSXT SFTP server desired Config",  # controller title as defined in compliance kit.
    ...
    def get(self, context: NSXTManagerContext, template: dict = None) -> Tuple[Any, List[str]]:
        # For sftp server in NSX versions < 3.X, we must use the mgmt api.  In NSX versions > 4 the policy api can also be used.
        errors = []
        try:
            ...
            logger.exception(f"Exception retrieving backup configuration from NSX-T - {e}")
            errors.append(str(e))
        nsxt_backup_enabled = response.get("backup_enabled","")
        return {"enabled": nsxt_backup_enabled}, errors       
    ...
    def remediate(self, context: NSXTManagerContext, desired_values: Any) -> Dict:
        """Remediate current backup_and_restore config drift.

        :param context: Product context instance.
        :type context: NSXTManagerContext
        :param desired_values: Desired values for backup_and_restore config.
        :type desired_values: Any
