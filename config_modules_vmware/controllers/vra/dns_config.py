...
class DnsConfig(BaseController):
    ...
    | Config Id - 0000
    | Config Title - Placeholder title for VRA DNS control

    """

    metadata = ControllerMetadata(
        name="dns",  # controller name
        path_in_schema="compliance_config.vra.dns",  # path in the schema to this controller's definition.
        configuration_id="0",  # configuration id as defined in compliance kit.
        title="Placeholder title for VRA DNS control",
    ...
    def check_compliance(self, context: VraContext, desired_values: Any) -> Dict:
        ...
        :rtype: dict
        """
        # vra only needs servers for DNS control
    ...
    def set(self, context: VraContext, desired_values: Dict) -> Tuple[str, List[Any]]:
        ...
        try:
            ...
            logger.debug(f"Current VRA DNS servers: {current_dns_servers}")
            logger.debug(f"Desired VRA DNS servers: {desired_dns_servers}")
