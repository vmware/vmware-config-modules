...
class SshX11ForwardingPolicy(BaseController):
    ...
    metadata = ControllerMetadata(
        name="ssh_x11_forwarding",  # controller name
        path_in_schema="compliance_config.esxi.ssh_x11_forwarding",
        # path in the schema to this controller's definition.
        configuration_id="14",  # configuration id as defined in compliance kit.
    ...
    def get(self, context: HostContext) -> Tuple[str, List[str]]:
        ...
        :return: Tuple of str for 'x11forwarding' value and a list of error messages.
        :rtype: Tuple
        """
        logger.info("Getting ssh host x11 forwarding policy for esxi.")
        if not utils.is_newer_or_same_version(context.product_version, "8.0.0"):
            return "", [consts.SKIPPED]
        else:
            errors = []
            x11_forwarding_settings = ""
            try:
                x11_forwarding_settings = esxi_ssh_config_utils.get_ssh_config_value(context, CONFIG_KEY)
            except Exception as e:
                logger.exception(f"An error occurred: {e}")
                errors.append(str(e))
            return x11_forwarding_settings, errors
    ...
    def set(self, context: HostContext, desired_values) -> Tuple[RemediateStatus, List[str]]:
        ...
        :type context: HostContext
        :param desired_values: Desired value for 'x11forwarding' config.
        :type desired_values: str
        :return: Tuple of "status" and list of error messages.
        :rtype: Tuple
        """
        logger.info("Setting ssh host x11 forwarding policy for esxi.")
        if not utils.is_newer_or_same_version(context.product_version, "8.0.0"):
    ...
    def check_compliance(self, context: HostContext, desired_values: str) -> Dict:
        ...
        :type context: HostContext
        :param desired_values: Desired value for the ssh host x11 forwarding settings.
        :type desired_values: str
        :return: Dict of status and current/desired value(for non_compliant) or errors (for failure).
        :rtype: dict
        """
        logger.debug("Checking compliance.")
        x11_forwarding_settings, errors = self.get(context=context)
        return esxi_ssh_config_utils.check_compliance_for_ssh_config(
            current_value=x11_forwarding_settings, desired_value=desired_values, errors=errors
        )
