...
class SnmpServicePolicy(BaseController):
    ...
    metadata = ControllerMetadata(
        name="snmp_service_policy",  # controller name
        path_in_schema="compliance_config.esxi.snmp_service_policy",
        # path in the schema to this controller's definition.
        configuration_id="1128",  # configuration id as defined in compliance kit.
    ...
    def get(self, context: HostContext) -> Tuple[dict, List[str]]:
        ...
        :rtype: Tuple
        """
        logger.info("Getting snmp service status for esxi.")
        errors = []
        snmp_service_status = {}
        try:
            host_service = context.host_ref.configManager.serviceSystem
            util = HostServiceUtil(host_service)
            snmp_service_status, errors = util.get_service_status(ESXI_SERVICE_SNMP)
        except Exception as e:
            logger.exception(f"An error occurred: {e}")
            errors.append(str(e))
        return snmp_service_status, errors
    ...
    def set(self, context: HostContext, desired_values) -> Tuple[RemediateStatus, List[str]]:
        ...
        :rtype: Tuple
        """
        logger.info("Setting snmp service policy for esxi")
        errors = []
        status = RemediateStatus.SUCCESS
        try:
            host_service = context.host_ref.configManager.serviceSystem
            util = HostServiceUtil(host_service)
            util.start_stop_service(ESXI_SERVICE_SNMP, desired_values.get(SERVICE_RUNNING))
            util.update_service_policy(ESXI_SERVICE_SNMP, desired_values.get(SERVICE_POLICY))
        except Exception as e:
            logger.exception(f"An error occurred: {e}")
            errors.append(str(e))
