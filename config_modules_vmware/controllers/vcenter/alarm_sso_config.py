...
SSO_EVENT_ID = "com.vmware.sso.PrincipalManagement"


class AlarmSSOConfig(BaseController):
    ...
    metadata = ControllerMetadata(
        name="alarm_sso_account_actions",  # controller name
        path_in_schema="compliance_config.vcenter.alarm_sso_account_actions",  # path in the schema to this controller's definition.
        configuration_id="1219",  # configuration id as defined in compliance kit.
    ...
    def get(self, context: VcenterContext) -> Tuple[List[Dict], List[Any]]:
        ...
        result = []
        try:
            logger.info(f"Get all the alarms with eventId {SSO_EVENT_ID}.")
    ...
    def set(self, context: VcenterContext, desired_values: List[Dict]) -> Tuple[str, List[Any]]:
        ...
        :param context: Product context instance.
        :type context: VcenterContext
        :param desired_values: List of dict objects with alarm info for the sso account actions alarm configuration.
        ...
        for desired_alarm_value in desired_values:
            ...
            try:
                content = context.vc_vmomi_client().content
                spec = vc_alarms_utils.create_alarm_spec(desired_alarm_value, SSO_EVENT_ID)
