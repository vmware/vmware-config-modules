...
from config_modules_vmware.controllers.esxi.cim_service_policy import CimServicePolicy
...
class TestCimServicePolicy:
    def setup_method(self):
        self.controller = CimServicePolicy()
    ...
    def test_check_compliance_compliant2(self):
        self.host_service.serviceInfo.service = [MagicMock(key=ESXI_SERVICE_CIM, running=True, policy="off")]
        expected_result = {consts.STATUS: ComplianceStatus.COMPLIANT}
    ...
    def test_check_compliance_non_compliant(self):
        self.host_service.serviceInfo.service = [MagicMock(key=ESXI_SERVICE_CIM, running=True, policy="on")]
        expected_result = {
    ...
    def test_check_compliance_non_compliant2(self):
        self.host_service.serviceInfo.service = [MagicMock(key=ESXI_SERVICE_CIM, running=False, policy="on")]
        expected_result = {
            consts.STATUS: ComplianceStatus.NON_COMPLIANT,
