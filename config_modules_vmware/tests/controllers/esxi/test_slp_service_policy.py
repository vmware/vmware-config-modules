...
from config_modules_vmware.controllers.esxi.slp_service_policy import SlpServicePolicy
...
class TestSlpServicePolicy:
    def setup_method(self):
        self.controller = SlpServicePolicy()
    ...
    def test_check_compliance_compliant2(self):
        self.host_service.serviceInfo.service = [MagicMock(key=ESXI_SERVICE_SLP, running=True, policy="off")]
        expected_result = {consts.STATUS: ComplianceStatus.COMPLIANT}
    ...
    def test_check_compliance_non_compliant(self):
        self.host_service.serviceInfo.service = [MagicMock(key=ESXI_SERVICE_SLP, running=True, policy="on")]
        expected_result = {
    ...
    def test_check_compliance_non_compliant2(self):
        self.host_service.serviceInfo.service = [MagicMock(key=ESXI_SERVICE_SLP, running=False, policy="on")]
        expected_result = {
            consts.STATUS: ComplianceStatus.NON_COMPLIANT,
