...
from config_modules_vmware.controllers.esxi.ssh_service_policy import ESXI_SERVICE_SSH
from config_modules_vmware.controllers.esxi.ssh_service_policy import SshServicePolicy
...
class TestSshServicePolicy:
    def setup_method(self):
        self.controller = SshServicePolicy()
        ...
        self.host_service.serviceInfo.service = [MagicMock(key=ESXI_SERVICE_SSH, running=False, policy="off")]
    ...
    def test_check_compliance_compliant2(self):
        self.host_service.serviceInfo.service = [MagicMock(key=ESXI_SERVICE_SSH, running=True, policy="off")]
        expected_result = {consts.STATUS: ComplianceStatus.COMPLIANT}
    ...
    def test_check_compliance_non_compliant(self):
        self.host_service.serviceInfo.service = [MagicMock(key=ESXI_SERVICE_SSH, running=True, policy="on")]
        expected_result = {
    ...
    def test_check_compliance_non_compliant2(self):
        self.host_service.serviceInfo.service = [MagicMock(key=ESXI_SERVICE_SSH, running=False, policy="on")]
        expected_result = {
            consts.STATUS: ComplianceStatus.NON_COMPLIANT,
