...
from config_modules_vmware.controllers.esxi.pg_vss_forged_transmits_accept import FORGED_TRANSMITS_ACCEPT
from config_modules_vmware.controllers.esxi.pg_vss_forged_transmits_accept import PgVssForgedTransmitsAccept
...
class TestPgVssForgedTransmitsAccept:
    def setup_method(self):
        self.controller = PgVssForgedTransmitsAccept()
        self.compliant_value = False
        self.compliant_return_value = [{"vss_name": "vswitch-1", FORGED_TRANSMITS_ACCEPT: False}]
        self.non_compliant_return_value = [{"vss_name": "vswitch-1", FORGED_TRANSMITS_ACCEPT: True}]
        self.non_compliant_value = True
        self.mock_host_ref = MagicMock()
        self.mock_host_ref.name = 'host-1'
        self.vss = MagicMock()
        self.vss.name = "vswitch-1"
        self.vss.spec.policy.security.forgedTransmits = self.compliant_value
    ...
    def test_set_failed(self):
        ...
        self.vss.spec.policy.security.forgedTransmits = self.non_compliant_value
        status, errors = self.controller.set(HostContext(host_ref=self.mock_host_ref), self.compliant_value)
    ...
    def test_check_compliance_failed(self):
        ...
        expected_result = {
            ...
            consts.ERRORS: ["'int' object has no attribute 'config'"]
        }
        assert result == expected_result
