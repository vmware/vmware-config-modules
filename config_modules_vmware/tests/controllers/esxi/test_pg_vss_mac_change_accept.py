...
from config_modules_vmware.controllers.esxi.pg_vss_mac_change_accept import PgVssMacChangeAccept
from config_modules_vmware.framework.auth.contexts.esxi_context import HostContext
from config_modules_vmware.framework.clients.common import consts
from config_modules_vmware.framework.models.output_models.compliance_response import ComplianceStatus
from config_modules_vmware.framework.models.output_models.remediate_response import RemediateStatus

class TestPgVssMacChangeAccept:
    def setup_method(self):
        self.controller = PgVssMacChangeAccept()
        self.compliant_value = False
        self.compliant_return_value = [{"vss_name": "vswitch-1", "mac_change_accept": False}]
        self.non_compliant_return_value = [{"vss_name": "vswitch-1", "mac_change_accept": True}]
        self.non_compliant_value = True
        self.mock_host_ref = MagicMock()
        self.mock_host_ref.name = 'host-1'
        self.vss = MagicMock()
        self.vss.name = "vswitch-1"
        self.vss.spec.policy.security.macChanges = self.compliant_value
    ...
    def test_set_failed(self):
        ...
        self.vss.spec.policy.security.macChanges = self.non_compliant_value
        status, errors = self.controller.set(HostContext(host_ref=self.mock_host_ref), self.compliant_value)
