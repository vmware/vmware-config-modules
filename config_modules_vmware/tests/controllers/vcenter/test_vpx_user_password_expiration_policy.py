...
from config_modules_vmware.controllers.vcenter.vpx_user_password_expiration_policy import (
    VpxUserPasswordExpirationPolicy,
)
...
class TestVpxUserPasswordExpirationPolicy:
    def setup_method(self):
        self.controller = VpxUserPasswordExpirationPolicy()
        self.compliant_value = 30
        self.non_compliant_value = 500
    ...
    def test_get_failed(self, mock_vc_vmomi_client, mock_vc_context):
        expected_error = Exception("Failed to get VPX host password expiration policy")
    ...
    def test_set_failed_due_to_failed_update(self, mock_vc_vmomi_client, mock_vc_context):
        ...
        result, errors = self.controller.set(mock_vc_context, self.compliant_value)
        assert result == RemediateStatus.FAILED
        assert errors == ['Failed to set VPXD option value for '
