...
from config_modules_vmware.controllers.vcenter.sso_max_failed_login_attempts \
    import SSOMaxFailedLoginAttempts
from config_modules_vmware.framework.clients.common import consts
from config_modules_vmware.framework.models.output_models.compliance_response import ComplianceStatus
from config_modules_vmware.framework.models.output_models.remediate_response import RemediateStatus


class TestSSOMaxFailedLoginAttempts:
    def setup_method(self):
        self.controller = SSOMaxFailedLoginAttempts()
    ...
    def test_get_failed(self, mock_vc_vmomi_sso_client, mock_vc_context):
        expected_error = Exception("Failed to get SSO max Failed login attempts")

        mock_vc_vmomi_sso_client.get_max_failed_login_attempts.side_effect = expected_error
        mock_vc_context.vc_vmomi_sso_client.return_value = mock_vc_vmomi_sso_client
    ...
    def test_set_failed_due_to_exception(self, mock_vc_vmomi_sso_client, mock_vc_context):
        expected_error = Exception("Failed to get SSO Max failed login attempt")

        mock_vc_vmomi_sso_client.set_max_failed_login_attempts.side_effect = expected_error
        mock_vc_context.vc_vmomi_sso_client.return_value = mock_vc_vmomi_sso_client
    ...
    def test_check_compliance_non_compliant(self, mock_vc_vmomi_sso_client, mock_vc_context):
        ...
        mock_vc_vmomi_sso_client.get_max_failed_login_attempts.return_value = self.non_compliant_value
        mock_vc_context.vc_vmomi_sso_client.return_value = mock_vc_vmomi_sso_client
