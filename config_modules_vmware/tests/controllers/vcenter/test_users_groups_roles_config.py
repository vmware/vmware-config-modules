...
class TestUsersGroupsRolesConfig:
    def setup_method(self):
        ...
        self.mock_permission_group3.principal = "abc.com\\vpxd-ed126b8a-0c50-4451-9b48-c03778b71dd4"
        ...
        self.desired_values = [
            {"role": self.mock_roles[0].name, "name": "abc.com\\user1", "type": USER}
        ]
    ...
    def test_get_success(self, mock_vc_context):
        ...
        expected_result = [
            {"role": self.mock_roles[0].name, "name": "abc.com\\user1", "type": USER},
            {"role": self.mock_roles[0].name, "name": "test_domain_name\\group1", "type": GROUP},
            {"role": self.mock_roles[1].name, "name": "test_domain_name\\group1", "type": GROUP},
            {"role": self.mock_roles[1].name, "name": "test_domain_alias\\group2", "type": GROUP}
        ]
    ...
    @patch("config_modules_vmware.framework.auth.contexts.vc_context.VcenterContext")
    @patch("config_modules_vmware.framework.clients.vcenter.vc_vmomi_sso_client.VcVmomiSSOClient")
    def test_check_compliance_compliant_with_alias(self, mock_vc_vmomi_sso_client, mock_vc_context):
        mock_vc_vmomi_sso_client.get_all_domains.return_value = self.domain_mock_obj
        mock_vc_context.vc_vmomi_sso_client.return_value = mock_vc_vmomi_sso_client
        mock_vc_context.vc_vmomi_client.return_value.content = self.mock_content
        desired_values = [
            {"role": self.mock_roles[0].name, "name": "abc.com\\user1", "type": USER},
            {"role": self.mock_roles[0].name, "name": "test_domain_alias\\group1", "type": GROUP},
            {"role": self.mock_roles[1].name, "name": "test_domain_alias\\group1", "type": GROUP},
            {"role": self.mock_roles[1].name, "name": "test_domain_name\\group2", "type": GROUP}
        ]
        result = self.controller.check_compliance(mock_vc_context, desired_values)
    ...
    def test_check_compliance_non_compliant(self, mock_vc_vmomi_sso_client, mock_vc_context):
        ...
        assert result[consts.DESIRED] == [None, None, None]
        assert len(result[consts.CURRENT]) == len(expected_current)
    ...
    @patch("config_modules_vmware.framework.auth.contexts.vc_context.VcenterContext")
    @patch("config_modules_vmware.framework.clients.vcenter.vc_vmomi_sso_client.VcVmomiSSOClient")
    def test_check_compliance_failed_unexpected_user_group(self, mock_vc_vmomi_sso_client, mock_vc_context):
        mock_vc_vmomi_sso_client.get_all_domains.return_value = self.domain_mock_obj
        mock_vc_context.vc_vmomi_sso_client.return_value = mock_vc_vmomi_sso_client
        mock_vc_context.vc_vmomi_client.return_value.content = self.mock_content
        desired_values_with_invalid_name = [
            {"role": "TestRole1", "name": "abc.comuser1", "type": USER}
        ]
    ...
    @patch("config_modules_vmware.framework.auth.contexts.vc_context.VcenterContext")
    @patch("config_modules_vmware.framework.clients.vcenter.vc_vmomi_sso_client.VcVmomiSSOClient")
    def test_remediation_skipped(self, mock_vc_vmomi_sso_client, mock_vc_context):
        mock_vc_vmomi_sso_client.get_all_domains.return_value = self.domain_mock_obj
        mock_vc_context.vc_vmomi_sso_client.return_value = mock_vc_vmomi_sso_client
        mock_vc_context.vc_vmomi_client.return_value.content = self.mock_content
        ...
        assert result[consts.ERRORS] == [consts.REMEDIATION_SKIPPED_MESSAGE]
        assert result[consts.DESIRED] == [None, None, None]
        assert len(result[consts.CURRENT]) == len(expected_current)
