...
from config_modules_vmware.controllers.vcenter.vpx_sddc_deployed_compliance_kit_config import \
    VpxSDDCDeployedComplianceKitConfig
from config_modules_vmware.framework.clients.common import consts
from config_modules_vmware.framework.models.output_models.compliance_response import ComplianceStatus
from config_modules_vmware.framework.models.output_models.remediate_response import RemediateStatus


class TestVpxSDDCDeployedComplianceKitConfig:
    def setup_method(self):
        self.controller = VpxSDDCDeployedComplianceKitConfig()
        self.compliant_sddc_compliance_kit = "VCF-NIST-800-53"
        self.non_compliant_sddc_compliance_kit = "none"
    ...
    def test_get_success(self, mock_vc_vmomi_client, mock_vc_context):
        mock_vc_vmomi_client.get_vpxd_option_value.return_value = self.compliant_sddc_compliance_kit
        mock_vc_context.vc_vmomi_client.return_value = mock_vc_vmomi_client

        result, errors = self.controller.get(mock_vc_context)
        assert result == self.compliant_sddc_compliance_kit
        assert errors == []

    @patch("config_modules_vmware.framework.auth.contexts.vc_context.VcenterContext")
    @patch("config_modules_vmware.framework.clients.vcenter.vc_vmomi_client.VcVmomiClient")
    def test_get_failed(self, mock_vc_vmomi_client, mock_vc_context):
        expected_error = Exception("Failed to get SDDC compliance kit config")
    ...
    def test_set_success(self, mock_vc_vmomi_client, mock_vc_context):
        ...
        result, errors = self.controller.set(mock_vc_context, self.compliant_sddc_compliance_kit)
    ...
    def test_set_failed_due_to_failed_update(self, mock_vc_vmomi_client, mock_vc_context):
        ...
        result, errors = self.controller.set(mock_vc_context, self.compliant_sddc_compliance_kit)
        assert result == RemediateStatus.FAILED
        assert errors == ['Failed to set VPXD option value for config.SDDC.Deployed.ComplianceKit']
    ...
    def test_check_compliance_compliant(self, mock_vc_vmomi_client, mock_vc_context):
        ...
        result = self.controller.check_compliance(mock_vc_context, self.compliant_sddc_compliance_kit)
    ...
    def test_check_compliance_non_compliant(self, mock_vc_vmomi_client, mock_vc_context):
        expected_result = {
            ...
            consts.CURRENT: self.non_compliant_sddc_compliance_kit,
            consts.DESIRED: self.compliant_sddc_compliance_kit,
        }
        mock_vc_vmomi_client.get_vpxd_option_value.return_value = self.non_compliant_sddc_compliance_kit
        mock_vc_context.vc_vmomi_client.return_value = mock_vc_vmomi_client
    ...
    def test_remediate_skipped_already_desired(self, mock_vc_vmomi_client, mock_vc_context):
        ...
        result = self.controller.remediate(mock_vc_context, self.compliant_sddc_compliance_kit)
        assert result == expected_result
    ...
    def test_remediate_success(self, mock_vc_vmomi_client, mock_vc_context):
        expected_result = {
            ...
            consts.OLD: self.non_compliant_sddc_compliance_kit,
            consts.NEW: self.compliant_sddc_compliance_kit,
        }
    ...
    def test_remediate_set_failed(self, mock_vc_vmomi_client, mock_vc_context):
        ...
        mock_vc_vmomi_client.set_vpxd_option_value.return_value = self.non_compliant_sddc_compliance_kit
        mock_vc_vmomi_client.set_vpxd_option_value.side_effect = expected_error
        mock_vc_context.vc_vmomi_client.return_value = mock_vc_vmomi_client
