...
def _LoadVmodlType(name):
    ...
    with _lazyLock:
        for defMap, loadFn in [
            (_dataDefMap, LoadDataType),
    ...
    if val == None:
        ...
        if info.flags & F_LINK:
            ...
            result = "(%s) {\n%s\n%s}" % (
                ...
                ",\n".join(
                    [FormatObject(getattr(val, prop.name), prop, indent + 1) for prop in val._GetPropertyList()]
                ),
                indent * "   ",
            )
...
class ManagedObject(object):
        ...
        if len(kwargs) > 0:
            ...
            for k, v in list(kwargs.items()):
                try:
                    ...
                    raise TypeError("%s() got an unexpected keyword argument '%s'" % (Capitalize(info.name), k))
                if idx < len(posargs):
                    raise TypeError("%s() got multiple values for keyword argument '%s'" % (Capitalize(info.name), k))
                args[idx] = v
    ...
    def _GetPropertyList(cls, includeBaseClassProps=True):
        ...
        while cls != ManagedObject:
            ...
            result = [info for info in cls._propList if prop.setdefault(info.name, cls) == cls] + result
            cls = cls.__bases__[0]
        return result
    ...
    def _GetMethodList(cls):
        ...
        while cls != ManagedObject:
            ...
            result = [
                info for info in list(cls._methodInfo.values()) if meth.setdefault(info.name, cls) == cls
            ] + result
            cls = cls.__bases__[0]
        return result
    ...
    def __init__(self, **kwargs):
        for property in self._GetPropertyList():
            ...
            if issubclass(ptype, list):
                ...
                value = ""
            elif ptype is long or issubclass(ptype, int) or issubclass(ptype, float):
...
class Curry(object):
    ...
    def __get__(self, obj, type):
        if obj:
            ...
            return lambda *args, **kwargs: self.f(*(self.args + (obj,) + args), **kwargs)
        return self
...
def CreateAndLoadMethodFaultType():
    with _lazyLock:
        ...
        propInfo = {}
        propList = [LazyObject(name=p[0], typeName=p[1], version=p[2], flags=p[3]) for p in props]
        dic = {
    ...
    with _lazyLock:
        ...
        for p in props:
            ...
            if flags & F_LINK:
                ...
                obj = LazyObject(name=name, typeName=typeName, version=propVersion, flags=flags)
            propList.append(obj)
        propInfo = {}
        for info in propList:
            propInfo[info.name] = info
        ...
        if _allowGetSet == True:
            for property in propList:
                ...
                tGlobals = {}
                exec(  # nosec
                    'def Get%s(self): return getattr(self, "%s")' % (suffix, name)
                    + "\n"
...
def LoadManagedType(vmodlName, wsdlName, parent, version, props, methods):
    with _lazyLock:
        ...
        propInfo = {}
        methodInfo = {}
        propList = [LazyObject(name=p[0], typeName=p[1], version=p[2], flags=p[3], privId=p[4]) for p in props]
        dic = {
            "_wsdlName": wsdlName,
            "_propInfo": propInfo,
            "_propList": propList,
        ...
        for mVmodl, mWsdl, mVersion, mParams, mResult, mPrivilege, mFaults in methods:
            ...
            if mWsdl.endswith("_Task"):
                isTask = True
            params = tuple(
                [LazyObject(name=p[0], typeName=p[1], version=p[2], flags=p[3], privId=p[4]) for p in mParams]
            )
            info = LazyObject(
                name=mName,
                typeName=vmodlName,
...
def LoadEnumType(vmodlName, wsdlName, version, values):
    with _lazyLock:
        ...
        # instead of LazyType
        result = type(name, (Enum,), {"_wsdlName": wsdlName, "_version": version})
...
def CheckField(info, val):
    with _lazyLock:
        ...
        if val is None or (isinstance(val, list) and len(val) == 0):
            ...
            info.type is type
            and valType is type(Exception)
            or issubclass(info.type, int)
            and issubclass(valType, int)
            or issubclass(info.type, long)
            and (issubclass(valType, int) or issubclass(valType, long))
            or issubclass(info.type, float)
            and issubclass(valType, float)
            or issubclass(info.type, six.string_types)
            and issubclass(valType, six.string_types)
            or issubclass(info.type, six.binary_type)
            and issubclass(valType, six.binary_type)
        ):
            return
            ...
            if issubclass(valType, GetVmodlType(info.expectedType)):
                return
        raise TypeError('For "%s" expected type %s, but got %s' % (info.name, info.type.__name__, valType.__name__))
...
def GetServiceVersions(namespace):
    ...
    if version_info[0] >= 3:
        ...
        return sorted([v for (v, n) in six.iteritems(serviceNsMap) if n == namespace], compare)
...
def _SetWsdlMethod(ns, wsdlName, inputMM):
    ...
    if isinstance(inputMM, list):
        if curMM is None:
            ...
            raise RuntimeError(
                "Duplicate wsdl method %s %s (new class %s vs existing %s)" % (ns, wsdlName, inputMM[0], curMM[0])
            )
        else:
            return curMM
...
def GetVmodlNs(version):
    ...
    assert len(versionParts) == 2, "Unsupported version format: %s" % version  # nosec
...
if not isinstance(bool, type):  # bool not a type in python <= 2.2
    bool = type("bool", (int,), {"__new__": lambda cls, val=0: int.__new__(cls, val and 1 or 0)})
byte = type("byte", (int,), {})
...
CreateManagedType(
    ManagedObject.__name__,
    ManagedObject._wsdlName,
    None,
...
CreateDataType(DataObject.__name__, DataObject._wsdlName, None, DataObject._version, [])
_AddType(DataObject)
...
class StringDict(dict):
    ...
    def __setitem__(self, key, val):
        ...
        if not isinstance(val, six.string_types):
            raise TypeError("key %s has non-string value %s of %s" % (key, val, type(val)))
