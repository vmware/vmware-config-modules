...
_dtExpr = re.compile(
    ...
    #       - BC (negative years)
    # year [-]0000..9999
    r"(?P<year>-?\d{4})"  # month 01..12
    r"(-(?P<month>(0[1-9]|1[0-2]))"  # day 01..31
    r"(-(?P<day>(0[1-9]|[1-2]\d|3[01])))?)?"  # time separator 'T'
    r"(T"  # hour 00..24
    r"(?P<hour>([01]\d|2[0-4]))"  # minute 00..59
    r"((:(?P<minute>[0-5]\d))"  # seconds 00..60 (leap second ok)
    r"(:(?P<second>([0-5]\d|60))"  # microsecond. max 16 digits
    ...
    r"(\.(?P<microsecond>\d{1,16}))?)?)?"  # UTC 'Z', or...
    r"((?P<tzutc>Z)"  # tz [+-]00..13:0..59|14:00
